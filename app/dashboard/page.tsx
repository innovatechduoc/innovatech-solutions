...
export default function InnovatechDashboard() {
  ...
  // --- MATEMÁTICAS DINÁMICAS PARA LOS KPIs ---

  // 1. Índice de Desempeño (Tasa de éxito basada en proyectos finalizados)
  const totalProyectos = activeProjects.length;
  const finalizados = activeProjects.filter(
    (p: any) => p.estado === "Finalizado" || p.estado === "Completado",
  ).length;
  const indiceDesempeno =
    totalProyectos > 0 ? Math.round((finalizados / totalProyectos) * 100) : 0;

  // 2. Proyectos Activos (Todos los que NO estén finalizados)
  const proyectosActivosCount = activeProjects.filter(
    (p: any) => p.estado !== "Finalizado" && p.estado !== "Completado",
  ).length;

  // 3. Horas Registradas (Suma de horasSemanales de todos los recursos)
  const horasTotales = resourceCapacity.reduce(
    (suma, recurso) => suma + (Number(recurso.horasSemanales) || 0),
    0,
  );

  // Reconstruimos el array de KPIs con los valores reales calculados
  const kpis = [
    {
      label: "Índice de Desempeño",
      value: `${indiceDesempeno}%`,
      icon: TrendingUp,
    ...
    },
    {
      ...
      value: proyectosActivosCount.toString(),
      icon: FolderKanban,
      color: "text-blue-600",
      bg: "bg-blue-100",
    },
